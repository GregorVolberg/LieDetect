...
event = event(ismember({event.value}, {'S 13'})); % keep only 
...
protocol.protocol.crime_scene = cellstr(protocol.protocol.crime_scene);
...
% no data personal_q_nr(ismember(protocol.protocol.role, 'self'))  = personal_q + 1; % 1 is incorrect, 2 is correct
