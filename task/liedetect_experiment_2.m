...
VpixxMarker21  = @(windowPointer) Screen('FillRect', windowPointer, [21 0 0], topLeftPixel); % viewpixx 
VpixxMarker31  = @(windowPointer) Screen('FillRect', windowPointer, [31 0 0], topLeftPixel); % viewpixx 
...
%         end
        ...
        for ntrial = 1:size(conmat, 1)
            ...
            % sucess = ismember({evt(end).value}, {'S 13'}) & (evt(end).sample > onset_sample(end));
            % if sucess 
...
%             RTsamp = round(RT * hdr.Fs);
%             if ~ isempty(evt(end).value) % necessary because "New Segment" event has empty value
%             sucess = ismember({evt(end).value}, {'S 31'}) & (evt(end).sample > onset_sample(end));
%             if sucess 
%                 %strt = GetSecs; WaitSecs(postmarker+0.3); % liest daten alle 0.25 sec 
%                 trialcount = trialcount + 1;
%                 onset_sample(trialcount) = evt(end).sample + RTsamp;
